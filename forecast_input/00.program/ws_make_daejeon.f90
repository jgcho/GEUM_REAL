...
!234567################################################################
      ...
      ! nw: 기상자료의 수 (기온, 습도, 기압, 바람u, 바람v, 일사)
...
subroutine READPNT
      open(12,file='./ws_point_daejeon.dat')
      read(12,*) 
      lm=0
...
subroutine READWTH
      character*5 cwth(nw)
      data cwth/'TMP','RH','PRES','u','v','NDNSW'/
...
subroutine WRITEWTH
      ...
      data cwthd/'TEM(C)','RH(%)','PRES(kPa)','WS(m/s)','SR(MJ/m2)'/

      read(cymd(1:4),'(i4)') iy
      read(cymd(5:6),'(i2)') mn
      read(cymd(7:8),'(i2)') idy
      
      write(cou,'(3a)') cdir,cymd,'.wda'
      open(52,file=cou)
      ...
      do l=0,Lmax ! LDAPS Start
        ...
        do ll=1,nwt ! 지점
          do n=1,nw-1 ! 항목
            ...
            elseif (n.eq.3) then ! Pres (kPa)
              WLwth(n,ll)=Lwth(Li(ll),Lj(ll),l,n)/1000.
            elseif (n.eq.4) then ! Wind speed (m/s)
        ...
        do ll=1,nwt
          do n=1,nw-1
            ...
            elseif (n.eq.3) then ! Pres (kPa)
              ...
              WRwth(n,ll)=Rwth(Ri(ll),Rj(ll),l+3,n)/1000.
            elseif (n.eq.4) then ! Wind speed (m/s)
